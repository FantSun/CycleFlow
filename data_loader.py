...
class Utterances(data.Dataset):
    ...
    def __init__(self, root_dir, feat_dir, dvec_dir, mode):
        """Initialize and preprocess the Utterances dataset."""
        self.root_dir = root_dir
        self.feat_dir = feat_dir
        self.dvec_dir = dvec_dir
        self.mode = mode
        self.step = 20
        self.split = 0
    ...
    def load_data(self, submeta, dataset, idx_offset, mode):
        for k, sbmt in enumerate(submeta):    
            ...
            dv_tmp = np.load(os.path.join(self.dvec_dir, sbmt[3]))
            if self.mode == 'train':
    ...
    def __getitem__(self, index):
        ...
        spk_org = list_uttrs[0]
        spk_id_org = list_uttrs[1]
        
        melsp, f0_org, emb_org = list_uttrs[2]
...
def get_loader(hparams):
    ...
    dataset = Utterances(hparams.root_dir, hparams.feat_dir, hparams.dvec_dir, hparams.mode)
